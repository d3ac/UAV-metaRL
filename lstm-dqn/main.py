...
if __name__ == '__main__':
    ...
    sample_length = 50 #TODO train的时候每次从agent的memory里sample多少个
    ...
    for i_episode in Trange:
        ...
        # ----------------- presample -----------------
        # generate hidden for presample
        hidden_all = []
        for i in range(env.n_ch):
            hidden = (torch.zeros(2, 1, 32).float(), torch.zeros(2, 1, 32).float())
            hidden_all.append(hidden)
        
        next_obs = env.reset(task)
        for step in range(n_pre_sample):
            obs = get_obs(env, next_obs)
            actions = [[], [], []]
            for i in range(env.n_ch):
                actions[i], hidden_all[i] = env.agents[i].get_action(obs[i], hidden_all[i], get_decay(i_episode))
