...
class Jammerchannels:
    ...
    def update_fast_fading(self):
        h = 1 / np.sqrt(2) * (np.random.normal(size=(self.n_jammer, self.n_uav, self.n_channel)) + 1j * np.random.normal(size=(self.n_jammer, self.n_uav, self.n_channel)))
        self.FastFading = 20 * np.log10(np.abs(h))
...
class UAV:
    def __init__(self, start_position, start_direction, start_velocity, start_p):
            ...
            self.uav_p = []
            self.destinations = []
            self.connections = [] # 与簇头之相连的无人机有哪些
...
class DRQN(nn.Module):
    def __init__(self, action):
        ...
        #self.env = Environ()
        self.input = 14   # 网络总输入:每个无人机簇的状态，8个快衰落（2个成员x4个信道）、6个信道（6个成员）、6个功率（6个成员）、1个干扰机信道状态
        self.lstm_i_dim = self.input    # input dimension of LSTM
        self.lstm_h_dim = 32    # output dimension of LSTM
...
class ReplayMemory(object):
    def __init__(self, max_epi_num=50, max_epi_len=300, sample_length=50):
        # capacity is the maximum number of episodes
        self.max_epi_num = max_epi_num
        self.max_epi_len = max_epi_len
        self.sample_length = sample_length
        self.memory = deque(maxlen=2000)   # TODO 这里写死了
    ...
    def sample(self):
        if self.is_available():
            epi_index = np.random.choice(range(len(self.memory)), self.sample_length, replace=False)
...
class Agent:
    ...
    def train(self):
        if self.buffer.is_available():
            ...
            hidden = (Variable(torch.zeros(2, 1, 32).float()), Variable(torch.zeros(2, 1, 32).float()))
    ...
    def get_params(self):
        return self.drqn.state_dict()
    
    def load_params(self, params):
        self.drqn.load_state_dict(params)
...
class Environ(gym.Env):
    def __init__(self):
        ...
        self.n_uav_pair = self.n_ch * self.n_des  # 一共6个通信对, 通信对是指簇头和簇成员之间的通信, 而且是簇内的
        self.n_jammer = 3  # number of jammers
        self.n_channel = 5  # int(self.n_ch+self.n_jammer-1)  # number of channels
        ...
        self.uav_pairs = np.zeros([self.n_ch, self.n_des, 2], dtype=np.int32)# 0是簇头的id, 1是簇成员的id
        self.uav_clusters = np.zeros([self.n_ch, self.n_cm_for_a_ch, 2], dtype=np.int32) # 0是簇头的id, 1是簇成员的id

        self.channel_range = self.n_channel
        self.power_range = len(self.uav_power_list)
        self.action_range = self.channel_range * self.power_range # 就是一个uav要选择两个东西, 一个是信道, 一个是功率
        self.action_dim = self.action_range ** self.n_des # 这个地方是簇头的actiondim, 也就是两个成员的actiondim * actiondim 种选择则
        ...
        #添加智能体
        n_steps = 1000
        self.agents = [Agent(i, self.action_dim, max_epi_num=200,max_epi_len=n_steps) for i in range(self.n_ch)] #TODO max_epi_num可调
    ...
    def get_params(self):
        ...
        for agent in self.agents:
            params.append(agent.get_params())
        return params
    ...
    def all_observed_states(self): # 生成转移矩阵, 创造一个环境只生成一次
    ...
    def init_channels(self):
        ...
        self.UAVchannels_with_fastfading = UAVchannels_with_fastfading - self.UAVchannels.FastFading # 就是要把pathloss减去fastfading, 先复制channel次, 然后每一个channel都减去fastfading
    ...
    def new_random_game(self):
        ...
        self.jammers = []
        self.init_uavs()       #更新簇头无人机的位置、方向和速度、夹角
        self.init_uav_clusters()       #更新簇头和簇内无人机的位置 并且保证簇内成员与簇头在通信范围内
        self.init_jammers()        #更新干扰机的位置
        ...
        if self.policy == "Q_learning":
            ...
            """
                channel state information (csi) :
                    shape (簇的个数3, 通信对象&簇成员数2, 信道数5)
                uav channels and uav powers: 通信的信道, 功率是啥
                    shape (簇的个数3, 通信对象&簇成员数2)
                jammer channels:
                    shape (干扰机数3)

            """
            joint_state = []
            csi = np.zeros([self.n_ch, self.n_des, self.n_channel]) # channel state information
            self.uav_jump_count = np.zeros([self.n_ch], dtype=np.int32)
            ...
            jammer_channels = np.asarray([x / self.n_channel for x in self.jammer_channels]) # jammerchannels 是 jammer所有state中抽出来的某一个, 着一个里面就包含了所有jammer到底干扰哪一个信道
            for i in range(self.n_ch): # 簇头个数
                # 因为reshape成-1了,所以就直接接起来的
                joint_state.append(np.concatenate((csi[i].reshape([-1]), uav_channels.reshape([-1]), uav_powers.reshape([-1]), jammer_channels)).astype(np.float32)) 
            for i in range(self.n_ch):
                joint_state[i].reshape([-1, 1])
            temp_state = np.stack(joint_state, axis=0)
            return joint_state
    # 没看
    ...
    def renew_jammer_channels_after_Rx(self):
        ...
        if np.floor_divide((self.t_jammer - self.t_Rx), self.t_dwell) == np.floor_divide(self.t_jammer, self.t_dwell) - 1: #在一次传输Rx之中
        # （干扰机时间-传输时间0.98）/干扰机扫频停留时间2.28 == 干扰机时间/干扰机扫频停留时间 - 1
            if self.type_of_interference == "saopin":
                ...
                if self.t_jammer % self.t_dwell == 0:  #DEBUG 传输完成后切换干扰信道, 从来没有进入过
                    for i in range(self.n_jammer):
                    ...
                    self.jammer_time[0] = self.t_jammer - change_point  # 0对应传输后半段的干扰时间
                    self.jammer_time[1] = self.t_Rx - self.jammer_time[0] # TODO 这个是干嘛的
    ...
    def get_action(self, action):
            if self.policy == "Q_learning":
                ...
                for i in range(self.n_ch):
                    for j in range(self.n_ch - 1, -1, -1):  # 从n_ch往前
