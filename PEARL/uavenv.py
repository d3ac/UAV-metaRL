...
class Environ(gym.Env):
    def __init__(self):
        ...
        self.n_channel = 4  # int(self.n_ch+self.n_jammer-1)  # number of channels
        ...
        self.action_space = spaces.MultiDiscrete([self.action_dim for _ in range(self.n_ch)])  # 网络
        #与奖励相关参数
        ...
        # 网络
        self.all_observed_states() # 生成转移矩阵的.

        self.n_task = 100
        self.generate_task() # 覆盖一次
        self.reset_task(0)
        self.observation_space = spaces.Box(low=-np.inf, high=+np.inf, shape=(self.get_state().shape[0],));
    ...
    def generate_task(self):
        ...
        self.all_jammer_states_list.extend(list(permutations(self.channel_indexes, self.n_jammer)))
        self.jammer_state_dim = int(perm(self.n_channel, self.n_jammer))

        for i in range(self.n_task):
            p_trans = np.random.uniform(0, 1, [self.jammer_state_dim, self.jammer_state_dim])
            ...
            self.p_trans_array.append(p_trans)
    ...
    def get_state(self):
        if self.policy == "Q_learning":
            ...
            joint_state = np.concatenate(joint_state, axis=0)
            return joint_state
    # 没看
    ...
    def renew_jammer_channels_after_collect(self):
        ...
        if np.floor_divide((self.t_jammer - self.t_collect), self.t_dwell) == np.floor_divide(self.t_jammer, self.t_dwell) - 1:
            if self.type_of_interference == "saopin":
                for i in range(self.n_jammer):
                    self.jammer_channels[i] += self.step_forward
                    self.jammer_channels[i] = int(self.jammer_channels[i] % self.n_channel)
    ...
    def reset_task(self, idx):
        self.p_trans = self.p_trans_array[idx]
        self.new_random_game()
