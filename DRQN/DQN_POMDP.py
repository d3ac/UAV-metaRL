...
def plot(cost_list):
    ...
    np.savetxt('DQN_po.txt',y_data[0],fmt='%f')

    plt.plot(x_data, y_data[0])
    plt.ylabel('DQN-PO-reward')
    ...
    for i in range(episodes):
        ...
        for t in range(max_step):
            ...
            for j in range(env.n_ch):
                ...
                obs[j] = np.hstack((s[j][(env.n_channel * env.n_des + env.n_des * j):(
                            env.n_channel * env.n_des + env.n_des + env.n_des * j)],
                                    s[j][(env.n_channel * env.n_des + env.n_ch * env.n_des + env.n_des * j):(
            ...
            for j in range(env.n_ch):
                ...
                # obs_prime[j] = s[j][8:]
                obs_prime[j] = np.hstack((s_prime[j][(env.n_channel * env.n_des + env.n_des * j):(
                            env.n_channel * env.n_des + env.n_des + env.n_des * j)],
                                    s_prime[j][(env.n_channel * env.n_des + env.n_ch * env.n_des + env.n_des * j):(
                                                env.n_channel * env.n_des + env.n_ch * env.n_des + env.n_des + env.n_des * j)]))
